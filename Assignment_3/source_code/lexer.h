...
public:
    Lexer();
    vector<Token> lexical (string, int&);
    void push_token(Token&, int&, string&, char, char, vector<Token>&);
...
void Lexer::push_token(Token &access, int &state, string &curr_lex, char curr_char, char next_char, vector<Token> &tokens) {
...
vector<Token> Lexer::lexical(string curr_line, int &lineNum) {
